...
ON krai_intelligence.error_codes(
  error_code, 
  manufacturer_id, 
...
ON krai_intelligence.error_codes(product_id) WHERE product_id IS NOT NULL;

CREATE INDEX IF NOT EXISTS idx_error_codes_video_id 
ON krai_intelligence.error_codes(video_id) WHERE video_id IS NOT NULL;

CREATE INDEX IF NOT EXISTS idx_error_codes_document_id 
ON krai_intelligence.error_codes(document_id) WHERE document_id IS NOT NULL;

-- Create composite index for common queries
CREATE INDEX IF NOT EXISTS idx_error_codes_lookup
ON krai_intelligence.error_codes(error_code, manufacturer_id, product_id);
