...
async def get_connection():
    ...
    if not supabase_url:
        print("❌ Missing SUPABASE_URL in .env.database")
        sys.exit(1)
    
    if not db_password:
        print("❌ Missing SUPABASE_DB_PASSWORD in .env.database")
...
async def export_schema(conn):
    ...
    with open(schema_file, 'w', encoding='utf-8') as f:
        ...
        # Export each schema
        for schema in schemas:
            print(f"📦 Exporting schema: {schema}")
...
async def export_seed_data(conn):
    ...
    with open(seed_file, 'w', encoding='utf-8') as f:
        ...
        for full_table, limit in tables_to_export:
            ...
            print(f"📦 Exporting: {full_table} (limit: {limit})")
