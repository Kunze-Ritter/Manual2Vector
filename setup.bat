...
if errorlevel 1 (
    echo ❌ Error: PowerShell is required but not available on this system.
    exit /b 1
)
...
if not defined PS_VERSION_MAJOR (
    echo ❌ Error: Unable to determine PowerShell version.
    exit /b 1
)
...
if %PS_VERSION_MAJOR% LSS 5 (
    echo ❌ Error: PowerShell 5.0 or higher is required for RSA key generation (ExportPkcs8PrivateKey).
...
if /I "!RSA_EXPORT_STATUS!"=="SUPPORTED" (
    ...
    if errorlevel 1 (
        echo ❌ Error: PowerShell !PS_VERSION_MAJOR!.x (!PS_EDITION!) lacks ExportPkcs8PrivateKey and openssl.exe is not available on PATH.
...
>> .env echo.
>> .env echo # Optional (deprecated) legacy alias used by older scripts — prefer DATABASE_CONNECTION_URL
...
if %ENV_SIZE% LSS 1000 (
    echo ❌ Warning: .env file seems incomplete ^(size: %ENV_SIZE% bytes^)
...
if /I "%RSA_GENERATION_METHOD%"=="POWERSHELL" (
    ...
    if errorlevel 1 (
        echo ❌ Error: OpenSSL failed to generate RSA private key.
    ...
    if errorlevel 1 (
        echo ❌ Error: OpenSSL failed to export RSA private key in PKCS8 format.
        del "%RSA_PRIVATE_TEMP%" "%RSA_PRIVATE_DER%" "%RSA_PUBLIC_DER%" 2>nul
        exit /b 1
    )
    openssl rsa -in "%RSA_PRIVATE_TEMP%" -pubout -outform DER -out "%RSA_PUBLIC_DER%" >nul 2>&1
    if errorlevel 1 (
        echo ❌ Error: OpenSSL failed to export RSA public key.
        del "%RSA_PRIVATE_TEMP%" "%RSA_PRIVATE_DER%" "%RSA_PUBLIC_DER%" 2>nul
        exit /b 1
    )
...
if errorlevel 255 (
    echo ❌ Error: .env already exists and no interactive console is available to confirm overwrite.
