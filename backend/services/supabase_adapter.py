...
Concrete implementation of DatabaseAdapter for Supabase Cloud.
Supports both PostgREST API and direct PostgreSQL connections.
"""
...
class SupabaseAdapter(DatabaseAdapter):
    ...
    Supports dual access methods:
    - PostgREST API (via Supabase client)
    - Direct PostgreSQL (via asyncpg pool)
    """
    ...
    def __init__(self, supabase_url: str, supabase_key: str, postgres_url: Optional[str] = None, service_role_key: Optional[str] = None):
        ...
        self.logger = logging.getLogger("krai.database.supabase")
    ...
    async def connect(self):
        ...
        try:
            ...
            if not self.pg_pool and not self.service_client:
                self.logger.warning("  No cross-schema access available - Image deduplication will be limited")
    ...
    async def test_connection(self) -> bool:
        ...
        try:
            ...
            # Simple query to test connection - use a table that always exists
            result = self.client.table("vw_documents").select("id").limit(1).execute()
    ...
    async def create_image(self, image: ImageModel) -> str:
        ...
        try:
            ...
            if image.image_hash:
                existing_image = await self.get_image_by_hash(image.image_hash)
                if existing_image:
                    self.logger.info(f"Image with hash {image.image_hash[:16]}... already exists: {existing_image['id']}")
    ...
    async def create_intelligence_chunk(self, chunk: IntelligenceChunkModel) -> str:
        ...
        chunk_data = chunk.model_dump(mode='json')
        return await self.create_intelligence_chunk_from_dict(chunk_data)
    ...
    async def search_embeddings(self, query_embedding: List[float], limit: int = 10, match_threshold: float = 0.7, match_count: int = 10) -> List[Dict[str, Any]]:
        ...
        try:
            # Use Supabase's vector search RPC function
            result = self.client.rpc('search_embeddings', {
                'query_vector': query_embedding,
                'match_threshold': match_threshold,
                'match_count': match_count or limit
            }).execute()
    ...
    async def log_audit_event(self, event: AuditLogModel) -> str:
        ...
        try:
            ...
            self.logger.debug(f"Audit event: {event.action} on {event.entity_type}:{event.entity_id}")
            return "audit_disabled"

        except Exception as e:
            # Silently ignore audit logging errors
            self.logger.warning(f"Audit logging error: {e}")
            return "audit_error"
    ...
    async def check_embedding_exists(self, chunk_id: str) -> bool:
        """Check if embedding exists for a chunk_id - SQL or PostgREST"""
        try:
            # Method 1: Direct PostgreSQL
            if self.pg_pool:
                try:
            ...
            # Method 2: PostgREST via vw_embeddings view
            if self.service_client:
                try:
    ...
    # Intelligence Chunk Methods (additional implementation)
    async def create_intelligence_chunk_from_dict(self, chunk_data: Dict[str, Any]) -> Optional[str]:
    ...
    async def create_link(self, link_data: Dict[str, Any]) -> str:
        ...
        try:
            ...
            raise RuntimeError(f"Cannot create link: {e}")
    ...
    async def create_video(self, video_data: Dict[str, Any]) -> str:
        ...
        try:
            ...
            result = client.rpc('upsert_video', {
                'p_link_id': video_data.get('link_id'),
                'p_youtube_id': video_data.get('youtube_id'),
                'p_title': video_data.get('title'),
                'p_description': video_data.get('description'),
                'p_thumbnail_url': video_data.get('thumbnail_url'),
                'p_duration': video_data.get('duration'),
                'p_channel_id': video_data.get('channel_id'),
                'p_channel_title': video_data.get('channel_title'),
                'p_published_at': video_data.get('published_at'),
                'p_metadata': video_data.get('metadata', {})
            }).execute()
            
            if result.data:
                video_id = str(result.data)
                self.logger.info(f"Created/updated video: {video_id} (YouTube: {video_data.get('youtube_id')})")
                return video_id
            ...
            raise RuntimeError(f"Cannot create video: {e}")
    ...
    async def create_print_defect(self, defect: PrintDefectModel) -> str:
        ...
        try:
            result = self.client.table('vw_print_defects').insert(defect_data).execute()
            
            if result.data:
                defect_id = result.data[0]['id']
                self.logger.info(f"Created print defect {defect_id}")
                return defect_id
            else:
                raise Exception("Failed to create print defect")
                
        except Exception as e:
            self.logger.error(f"Failed to create print defect: {e}")
            raise RuntimeError(f"Cannot create print defect in database: {e}")
    
    # Missing methods from DatabaseAdapter interface
    async def get_chunk_by_document_and_index(self, document_id: str, chunk_index: int) -> Optional[Dict[str, Any]]:
        ...
        try:
            result = self.client.table('vw_chunks').select('*').eq('document_id', document_id).eq('chunk_index', chunk_index).execute()
            return result.data[0] if result.data else None
        except Exception as e:
