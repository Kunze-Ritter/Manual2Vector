...
async def test_database_views():
    ...
    try:
        ...
        result = await adapter.execute_query("SELECT * FROM public.vw_pipeline_metrics_aggregated LIMIT 1")
        if result:
            print(f"   ✅ View exists and returns data")
            print(f"      Total docs: {result[0].get('total_documents', 0)}")
        else:
            print("   ⚠️  View exists but no data")
        
        # Test queue view
        print("\n2. Testing vw_queue_metrics_aggregated...")
        result = await adapter.execute_query("SELECT * FROM public.vw_queue_metrics_aggregated LIMIT 1")
        if result:
            print(f"   ✅ View exists and returns data")
            print(f"      Total items: {result[0].get('total_items', 0)}")
        else:
            print("   ⚠️  View exists but no data")
        
        # Test stage view
        print("\n3. Testing vw_stage_metrics_aggregated...")
        result = await adapter.execute_query("SELECT * FROM public.vw_stage_metrics_aggregated")
        if result:
            print(f"   ✅ View exists and returns data")
            print(f"      Stages: {len(result)}")
        else:
            print("   ⚠️  View exists but no data")
        
        # Test RPC functions via direct SQL
        print("\n4. Testing RPC Functions...")
        try:
            result = await adapter.execute_query("SELECT * FROM get_duplicate_hashes()")
            print(f"   ✅ get_duplicate_hashes: {len(result or [])} results")
        except Exception as e:
            print(f"   ❌ get_duplicate_hashes failed: {e}")
        
        try:
            result = await adapter.execute_query("SELECT * FROM get_duplicate_filenames()")
            print(f"   ✅ get_duplicate_filenames: {len(result or [])} results")
        except Exception as e:
